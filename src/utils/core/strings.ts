...
export const capitalizeWords = (input: string, titleCase = false, ignoreWordsList: string[] = []): string => {
  ...
  const isIgnored = (word: string): boolean => {
    return Boolean(ignoreWordsList.some(iw => iw.toLowerCase() === word.toLowerCase())) ?? Boolean(TITLECASE.includes(word.toLowerCase()));
  };
 ...
 * @param ignoreWordsList - An optional array of words to not capitalize in the output.
 *
 * @returns The input string with the first letter of each word capitalized, except for ignored words.
 */
export const titleCase = (input: string, ignoreWordsList: string[] = []) => {
  return capitalizeWords(input, true, ignoreWordsList);
};
